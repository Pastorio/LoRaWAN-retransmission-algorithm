...
//    uint8_t enviar_dado[3] = { highByte(dado), lowByte(dado)};
    ...
    Serial.println((String)"| count_retrans: "+count_retransmission+" | fcnt_reenviar: " + fcnt_reenviar+ " | dado: " + dado + " | ");
    ...
    int rand_block = rand() % 100 + 1;
    Serial.println((String)"| rand_block: " + rand_block);
    if (rand_block <= 20) {
      Serial.println(F("XX RETRANSMISSION NOT QUEUED XX"));
      Serial.println((String)"|Mensagem: " + dado+ "fcnt: " + fcnt + " | fcnt_reenviar: "+ fcnt_reenviar+"|");
      Serial.println("XX --------------------------- XX");
      count_retransmission_block++;
      return ;
      }
...
//        LMIC.seqnoUp = 1;
    ...
    int rand_block = rand() % 100 + 1;
    Serial.println((String)"| rand_block: " + rand_block);
    if (LMIC.seqnoUp > 5 && rand_block <= 20) {
      int fcnt = LMIC.seqnoUp;
      LMIC.seqnoUp  = LMIC.seqnoUp+1;
      int dado = rand() % 100 + 1;
      registro_mensagem.push_back({fcnt, dado});
      Serial.println(F("XX PACKET NOT QUEUED XX"));
      Serial.println((String)"| Mensagem: " + dado+" fcnt: " + fcnt + " | SeqNo: "+ LMIC.seqnoUp+"|");
      Serial.println("XX ------------------ XX");
      count_block++;
      }
     Serial.println((String)"| count_block: " + count_block+" count_retransmission_block: " + count_retransmission_block);
