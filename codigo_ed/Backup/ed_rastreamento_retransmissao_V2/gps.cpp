...
void gps::buildPacket(uint8_t txBuffer[9], double gps_lat, double gps_long)
{

  LatitudeBinary = ((gps_lat + 90) / 180.0) * 16777215;
  LongitudeBinary = ((gps_long + 180) / 360.0) * 16777215;
  ...
  sprintf(t, "Alt: %f", tGps.altitude.meters());
  Serial.println(t);
  ...
  sprintf(t, "Hdop: %f", tGps.hdop.value());
  Serial.println(t);
...
void gps::get_coordenates(double *latitude, double *longitude){
