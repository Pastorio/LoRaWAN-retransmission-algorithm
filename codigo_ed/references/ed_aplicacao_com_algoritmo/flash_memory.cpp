...
void flash_memory::save_data(uint32_t seqNum, float lat_data, float long_data)
{
  ...
  long_data = (float) long_data;

  unsigned int memory_pos = preferences.getUInt("memory_pos", 0);

  Serial.println((String) "SAVE Seq Num: " + seqNum + " " + lat_data + " " + long_data + " mem- " + memory_pos + " lenMem: " + lenPrefMemory);
  ...
  if ((memory_pos < lenPrefMemory)  | (memory_pos == 0)) {
    memory_pos++;
  } else {
    memory_pos = 0;
  }
...
void flash_memory::get_data(uint32_t seqNum, float *latitude, float *longitude) {
  ...
  int pos;
  Serial.println((String) " SEQNUM " + seqNum);
  for (pos = 0; pos < lenPrefMemory; pos++) {
    if (bufferSeqNum[pos] == seqNum) {
      break;
    }
  }
  
  if (pos == lenPrefMemory) {
    *latitude = 0;
    *longitude = 0;
  }
  else {
    ...
    float bufferLat[latLen];
    preferences.getBytes("latitude", bufferLat, latLen);
    *latitude = bufferLat[pos];
    
    // Get longitude
    size_t longLen = preferences.getBytesLength("longitude");
    float bufferLong[longLen];
    preferences.getBytes("longitude", bufferLong, longLen);
    *longitude = bufferLong[pos];
  }
